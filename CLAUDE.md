...
- **Irregularity on the answer card** (1.23): a verb card carries `flag: 'irregular'` (rendered as a tag by the pronunciation in quiz.js and daily.js — a generic optional card field, any app may set one) when its form differs from the oracle's regular expectation (`regularExpectation()` in topics.js; the subjunctive is compared with what a *regular* perfeito 3pl would derive to). The reveal's conjugation table highlights the differing letters per row (`irregularSpan()`: longest common prefix/suffix against the expected form — `faço` → `ç`; a pure dropped ending like `quer` gets a dashed gap) and a `.conj-note` line names what the pattern would have given. Orthographic shifts the oracle knows (`conheço`, `cheguei`) count as regular; `pôr` has no oracle and is flagged whole. Browse's expanded conjugation panels (`conjPanelHtml` in browse.js) reuse the same helpers, with a one-line legend counting the irregular forms.
- **Synonym variants** (1.23.4): a verb card whose lexeme is in `synonyms` (verbs.js: `pôr`/`botar`/`colocar`, `andar`/`caminhar`) accepts the sister verbs' forms and carries `variants` — one `{ accepted, answer, pron, flag, speak, reveal }` per sister. `cardFace(card, typed)` in `js/lib/text.js` (fed `matchAnswer()`'s `hit` on a hit, the raw typed text on a miss) swaps in the face of the verb the learner reached for: `coloco` for "I put" is answered as *eu coloco* with colocar's pronunciation and conjugation table, not pôr's; on a miss the face whose form shares the longest prefix with the typed word leads (`colocamos` → colocar), ties keep the card. The other synonyms follow in an `.also-tag` line ("also eu ponho · eu boto", `otherFaces()`, string `QUIZ_STRINGS.also`). Progress records stay on the card (the id is still `pôr|0`). `variants` is a generic optional card field any registry may set.
